...
fileNames = ["asc", "desc", "ascDesc", "descAsc", "stillAsc", "descStill"];
...
fAMmax = c3 * 2^(8/12);
fAMmult = 0.0000001; % Range multiplier e.g. fAMmult = 20 => fAm > 0 & fAm < 20
...
output1 = [];
allAudio = zeros(793800, 1);

% For every contour
for idx = 1 : size(noteContours1, 1)
    output1 = cat(1, output1, WriteSineNotes(allAudio(chunk : chunk + ...
        durationContour), fs, frequencyContours1(idx, :), durNote, amplitude));

    output1 = cat(1, output1, zeros(durNote * fs, 1));
...
for idx = 1 : size(noteContours2, 1)
    output2 = cat(1, output2, WriteSineNotes(allAudio(chunk : chunk + ...
        durationContour), fs, frequencyContours2(idx, :), durNote, amplitude));

    output2 = cat(1, output2, zeros(durNote * fs, 1));
...
% chunk = 1;
% for idx = 1 : size(noteContours1, 1)
%     audiowrite(strcat("1_", fileNames(idx), "_", instrument, ".wav"), ...
%         output1(chunk : chunk + durationContour, :), fs);
% 
%     audiowrite(strcat("2_", fileNames(idx), "_", instrument, ".wav"), ...
%         output2(chunk : chunk + durationContour, :), fs);
% 
%     chunk = chunk + durationContour;
% end

audiowrite("Sine_1.wav", output1, fs);
audiowrite("Sine_2.wav", output2, fs);
%% Generate textfiles
